...
class Paypal_ec {
	...
	private $PROXY_HOST = '';
	private $PROXY_PORT = '';
	private $API_username = "developer1_api1.client.com";
	private $API_password = "C85VJSTMGDKDSLLN";
	private $API_signature = "AFcWxV21C7fd0v3bYYYRCpSSRl31A-gCfR3a9kPX66JEDDIdyH18bCtW";
	...
	*/
	function set_ec($ec_data = array(), $custom_data = false) {
		...
		if ($custom_data === false) {
            ...
            if(isset($ec_data['billing_type'])){ $nvpstr.= "&L_BILLINGTYPE0=" . urlencode($ec_data['billing_type']); }
            if(isset($ec_data['billingagreement_dec'])){ $nvpstr.= "&L_BILLINGAGREEMENTDESCRIPTION0=" . urlencode($ec_data['billingagreement_dec']); }
	...
	*/
    ...
    function recuring_ec(
		...
		if (isset($ec_details['PayerID'])) {
			$nvpstr.= '&PAYERID=' . urlencode($ec_details['PayerID']);
		}
		
		if (isset($ec_details['AMT'])) {
			$nvpstr.= '&AMT=' . urlencode($ec_details['AMT']);
		}
		if (isset($ec_details['CURRENCYCODE'])) {
			$nvpstr.= '&CURRENCYCODE=' . urlencode($ec_details['CURRENCYCODE']);
		}
		if (isset($ec_details['PROFILESTARTDATE'])) {
			$nvpstr.= '&PROFILESTARTDATE=' . urlencode($ec_details['PROFILESTARTDATE']);
		}
        if (isset($ec_details['DESC'])) {
			$nvpstr.= '&DESC=' . urlencode($ec_details['DESC']);
		}
        if (isset($ec_details['BILLINGPERIOD'])) {
			$nvpstr.= '&BILLINGPERIOD=' . urlencode($ec_details['BILLINGPERIOD']);
		}
        if (isset($ec_details['BILLINGFREQUENCY'])) {
			$nvpstr.= '&BILLINGFREQUENCY=' . urlencode($ec_details['BILLINGFREQUENCY']);
		}
        if (isset($ec_details['COUNTRYCODE'])) {
			$nvpstr.= '&COUNTRYCODE=' . urlencode($ec_details['COUNTRYCODE']);
		}
        if (isset($ec_details['MAXFAILEDPAYMENTS'])) {
			$nvpstr.= '&MAXFAILEDPAYMENTS=' . urlencode($ec_details['MAXFAILEDPAYMENTS']);
		}
		$nvpstr.= '&IPADDRESS=' . urlencode($_SERVER['SERVER_NAME']);
